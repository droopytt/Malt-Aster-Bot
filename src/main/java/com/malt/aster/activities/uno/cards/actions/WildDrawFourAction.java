...
public class WildDrawFourAction implements UnoActionable {
    ...
    public void perform(User sender, UnoMainGame unoMainGame) {
        StringBuilder stringBuilder = new StringBuilder(Utils.getEffectiveName(sender, unoMainGame.getGuild()) + " has put down a wild draw 4. Please wait for them to select the suit.");
    ...
    static void notifyWildCardToOtherParticipants(User sender, UnoMainGame unoMainGame, StringBuilder stringBuilder) {
        unoMainGame.getParticipants().stream().filter(user -> !user.equals(sender)).forEach(user -> user.openPrivateChannel()
                .queue(channel -> channel.sendMessage(stringBuilder).queue()));

        stringBuilder.append("\n\nPlease pick a suit name from the following\n");
        Arrays.stream(UnoSuit.values()).filter(unoSuit -> unoSuit != UnoSuit.WILD).forEach(suit -> stringBuilder.append(suit.toString().toLowerCase()).append("\n"));
        sender.openPrivateChannel().queue(privateChannel -> privateChannel.sendMessage(stringBuilder.toString()).queue());
