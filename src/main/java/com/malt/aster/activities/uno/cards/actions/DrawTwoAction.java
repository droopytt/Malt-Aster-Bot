...
public class DrawTwoAction implements UnoActionable {
    ...
    public void perform(User sender, UnoMainGame unoMainGame) {
        ...
        unoMainGame.getParticipants().forEach(participant -> participant.openPrivateChannel()
                .queue(channel -> channel.sendMessage(Utils.getEffectiveName(nextPlayer, unoMainGame.getGuild()) + " has drawn 2 cards and had their turn skipped!").queue()));
